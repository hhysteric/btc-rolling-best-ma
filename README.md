...
1. **4Y Rolling Best MA 时间序列** —— 拆成四张通版大图（纵向排列）：**单 MA、双 MA、单 EMA、双 EMA**。
   四条曲线挤在一张图里时，单均线的 200 天量级会把双均线快线的十几天量级压平，
   分图后每张纵轴各自自适应。左轴为每日最优均线周期（或切换为最优策略收益率），
   右轴为 BTC 收盘价（可切对数）。双均线那两张会画出快线与慢线两条参数曲线。
   四张图每张标题右侧都有「导出 Excel」，把该序列导成 `.xlsx`：单均线是日期 / 收盘价 / 最优周期 / 窗口收益率，
   双均线则拆成最优快线 / 最优慢线两列，另附一张记录计算配置与口径的说明表。面板顶部的「导出全部四张（Excel）」
   一次把已算出的四条序列各导一个文件。取不到 CDN 上的 xlsx 库时自动退回 UTF-8 BOM 的 CSV。
...
2. **自定义均线回测** —— 自己指定线型（MA/EMA）、单均线或双均线金叉、周期、时间区间与手续费，
   ...
   买入持有净值 = 价格 ÷ 区间首日收盘价，与价格线逐点同形；图上不与价格线重合是双轴各自缩放所致
   （右轴上界要同时容纳策略净值），两条线的相对高低不可比，比收益请看统计格里的「买入持有」与「超额」。
...
3. **4Y Rolling Best MA 四年大周期对比** —— 把同一条序列按四年大周期锚点对齐后叠在一起比较：
   ...
   横轴是「距锚点天数」，图例括号里是该轮锚点的**真实年月**（例如 `周期1 (2015.01)`，由数据定出来的那一天，
   最后一轮标「当前」因为锚点会随数据推进而移动）。菱形是本轮最新位置；不在图上标各轮极值——
   最优周期是逐日回看寻优的结果，「某轮最高」不构成参考位，对进行中的这一轮还常常落在第 0 天。
   极值数字在下方段落里给出，连同各轮均值、区间、峰值出现日与历史同位取值的对比
   （全部由数据算出，不含任何推测性表述）。
...
js/cache.js                   结果缓存（localStorage，按「配置 + 数据指纹」命中）
...
寻优结果只取决于两件事：计算配置（线型 / 模式 / 周期网格 / 手续费 / 窗口年数）与行情数据
（起点、终点、天数）。两者都没变，结果必然逐位相同，因此 `js/cache.js` 用「配置 + 数据指纹」
当 key，把各条序列的类型化数组以 base64 存进 `localStorage`（约 350KB），命中就**直接出图，不重算**，
页脚如实标注「结果取自本地缓存」。

数据后来长了几天（每日 Action 提交了新数据，或页面从 Binance 补到了新的一天）时，
先把缓存结果摆出来让页面立即可用，同时**后台补算**到最新日期，算完自动换成新结果并覆盖缓存——
不会拿旧值冒充最新，页脚会写「缓存结果算到 X 日，正在后台补算到 Y 日」。
点「重新计算」总是强制重算并刷新缓存。写入失败（隐私模式 / 配额满）时静默降级成每次重算，不影响出图。
