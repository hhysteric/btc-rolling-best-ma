...
1. **4Y Rolling Best MA 时间序列** —— 拆成四张图：**单 MA、双 MA、单 EMA、双 EMA**。
   四条曲线挤在一张图里时，单均线的 200 天量级会把双均线快线的十几天量级压平，
   分图后每张纵轴各自自适应。左轴为每日最优均线周期（或切换为最优策略收益率），
   右轴为 BTC 收盘价（可切对数）。双均线那两张会画出快线与慢线两条参数曲线。

2. **自定义均线回测** —— 自己指定线型（MA/EMA）、单均线或双均线金叉、周期、时间区间与手续费，
   在价格图上标出每一次买入（绿色上三角）与卖出（红色下三角），右轴给出策略净值与同区间买入持有净值，
   并列出策略收益率、买入持有、超额、最大回撤、成交笔数、回合胜率与最近 40 笔成交明细。
   规则与窗口寻优完全一致：满仓择时、按当日收盘价成交、每笔收一次单边手续费；
   均线用全历史数据算完再截区间（指标已预热），若区间开头历史仍不足周期长度，会如实提示有多少天未成形。

3. **4Y Rolling Best MA 四年大周期对比** —— 把同一条序列按四年大周期锚点对齐后叠在一起比较：
